...
{#if showInfo && $nextAdzanInfo}
	<div class="fixed bottom-4 right-4 bg-emerald-600 text-white p-4 rounded-lg shadow-lg max-w-sm z-50">
			...
			<div class="flex-1">
				...
				{:else}
					<p class="text-xs text-emerald-100">
						{$nextAdzanInfo.minutesUntilAdzan < 60 
							? `${$nextAdzanInfo.minutesUntilAdzan} menit lagi`
							: `${Math.floor($nextAdzanInfo.minutesUntilAdzan / 60)} jam ${$nextAdzanInfo.minutesUntilAdzan % 60} menit lagi`
						}
					</p>
				{/if}
